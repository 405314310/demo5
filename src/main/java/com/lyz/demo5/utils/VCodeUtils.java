...
public class VCodeUtils {
    ...
    public String getCheckCode() {
        String ZiMu = "abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRESTUVWXYZ0123456789";
        String result = "";
        Random random = new Random();
        for (int i = 0; i < 5; i++) {
    ...
    public boolean checkEmail(String email){
        ...
        if(email.indexOf("@")!=-1&&email.indexOf("@")==email.lastIndexOf("@")){
            if(email.indexOf("@")>0){
                System.out.println("有@ 且只有一个@ 并且@前面有字符");
                i++;
            }
        ...
        if(email.indexOf(".")!=-1&&email.indexOf(".")==email.lastIndexOf(".")){
            if(email.indexOf(".")> (email.indexOf("@")+1)) {
                System.out.println("有. 且只有一个. 并且.在@后面并不相邻");
                if(email.indexOf(".")!=email.length()){
                    System.out.println("老白说：.不是老末");
                    i++;
                }
            }
        }
